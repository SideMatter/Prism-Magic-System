...
function buildSystemPrompt(player: Player, accessibleSpells: Spell[]): string {
...
2. Suggest tactical spell combinations and strategies
3. Explain spell mechanics and interactions
4. Help with character building decisions
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Fetch all spells to build context
    const baseUrl = request.nextUrl.origin;
    const spellsResponse = await fetch(`${baseUrl}/api/spells`, {
      cache: "no-store",
    });
    ...
    const systemPrompt = buildSystemPrompt(player, accessibleSpells);
